function v_price = Q3a_AnalyticSolution()

s_price = 100:120;
v_price = arrayfun(@(x) calculate_analytic_solution(x), s_price);

figure
plot(s_price, v_price, 'b--o');
xlabel('Initial Stock Price');
ylabel('Anayltical Init Option Price');
end

function result = calculate_analytic_solution(s)
S_init = s; % the initial stock price
randn('state',100);
T = 0.50;
K = 102;
B = 100;
